...
var org1tlscacert_path = path.resolve(firstnetwork_path, 'crypto-config', 'peerOrganizations', 'org1.dmc.ajou.ac.kr', 'tlsca', 'tlsca.org1.dmc.ajou.ac.kr-cert.pem');
...
async function invoke() {
	...
	try {
		...
		// -- channel instance to represent the ledger named "dmcchannel"
		const channel = fabric_client.newChannel('dmcchannel');
		...
		const peer = fabric_client.newPeer('grpcs://localhost:7051', {
			'ssl-target-name-override': 'peer0.org1.dmc.ajou.ac.kr',
			pem: org1tlscacert
		});
